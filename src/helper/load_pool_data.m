...
while i<=n_pooled_days
    ...
    %check that there are sufficient number of validtrials per condition
    A_data = tidy_data(strcmp(tidy_data.trial_type,'A') & tidy_data.valid_tr,:);
    V_data =tidy_data(strcmp(tidy_data.trial_type,'V') & tidy_data.valid_tr,:);
    AV_data =tidy_data(strcmp(tidy_data.trial_type,'AV') & tidy_data.valid_tr,:);
